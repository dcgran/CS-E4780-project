...
\textbf{Hardware Environment.} Experiments were conducted on a machine with [TODO: e.g., ``Intel i7-12700H'' or ``Apple M2''], [TODO: e.g., ``16''] GB of memory, running [TODO: e.g., ``Ubuntu 22.04'' or ``macOS 14'']. However, hardware specifications have limited impact on end-to-end latency in this system, as the LLM inference---which dominates execution time---is performed remotely via API calls rather than locally.
...
\textbf{Benchmarking Infrastructure.} Timing data is collected using \texttt{time.perf\_counter()} around each pipeline stage. [TODO: Specify number of runs per query (e.g., 10), warm-up runs excluded (e.g., 1), and aggregation method (e.g., mean/median).]

\textbf{Memory Measurement.} [TODO: Either describe memory profiling method (e.g., tracemalloc, psutil for Kuzu DB footprint) OR state ``Memory profiling was not performed as LLM inference occurs remotely via API.'']
...
In cold runs, schema pruning is the clear bottleneck, responsible for over 80\% of total latency. On cache hits, pruning and query generation latency effectively disappear. Figure~\ref{fig:latency} visualizes this breakdown, showing the dramatic reduction from cold to cached execution. Cache hit rates averaged 10\% in our benchmark due to the diverse query set; production workloads with repeated queries would see substantially higher rates.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[fill=blue!70] coordinates {(Pruning,1447.3) (Text2Cypher,121.7) (Execution,4.4) (Response,199.0)};
      \addplot[fill=green!60] coordinates {(Pruning,0.01) (Text2Cypher,0.1) (Execution,2.6) (Response,3.8)};
  ...
  \Description{Bar chart showing latency per pipeline stage. Cold runs show Schema Pruning at 1447ms dominating, while cache hits reduce all stages to under 4ms.}
  \label{fig:latency}
\end{figure}
