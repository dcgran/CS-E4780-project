...
\textbf{Challenges.} One of the most challenging parts of the project was getting started with OpenCEP and finding the first matches to the query. At first, OpenCEP produced tons of ambiguous errors but once we had found the underlying issues, we did not face any major stumbling blocks with the programming. Another challenge was that the hot path query required an id for each trip but the Citibike dataset did not always include it. It was also unclear what the set of ids \lstinline|{7,8,9}| meant in the assignment since all of them did not exist in the dataset.
